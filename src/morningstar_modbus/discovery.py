...
"""Serial and bounded local TCP discovery."""
...
from morningstar_modbus.profiles import select_profile
from morningstar_modbus.transport import AsyncModbusRtuClient, AsyncModbusTcpClient, ReadOnlyModbusClient

LOGGER = logging.getLogger(__name__)


async def _probe(client: ReadOnlyModbusClient) -> tuple[DeviceIdentification, float] | None:
    ...
    for port in ports:
        ...
        for unit_id in config.watch.unit_ids:
            ...
            if result is not None:
                identity, latency_ms = result
                profile = select_profile(identity.vendor_name, identity.product_code).name
    ...
    async def probe_host(host: str, unit_id: int) -> DiscoveredDevice | None:
        async with semaphore:
            ...
            return DiscoveredDevice(
                ...
                identity,
                latency_ms,
                select_profile(identity.vendor_name, identity.product_code).name,
            )
