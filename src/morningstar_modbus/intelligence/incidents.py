...
Category = Literal["production", "charging", "battery", "communications", "data_integrity"]
...
class SiteIntelligenceService:
    """Continuously evaluate source-backed observations and reconcile incident lifecycle."""
    ...
    async def start(self) -> None:
        ...
        if self._task is None or self._task.done():
            self._task = asyncio.create_task(self._run(), name="morningstar-site-intelligence")
    ...
    async def scan_all(self) -> None:
        systems = await self.systems.list_systems()
    ...
    async def scan(self, system_uid: str) -> list[dict[str, object]]:
        async with self._scan_lock:
            ...
            snapshots = await asyncio.gather(
                *(self.systems.controllers_data.latest(str(item["controller_uid"])) for item in controllers)
            )
            for controller, sample in zip(controllers, snapshots, strict=True):
                uid = str(controller["controller_uid"])
                controller_findings, controller_evaluated = await self._evaluate_controller(
                    uid, controller, sample, now
                )
                findings.extend(controller_findings)
                evaluated.update(controller_evaluated)

            baseline = await self.solar_baseline(system_uid, now=now)
            baseline_key = f"solar_underproduction|{system_uid}"
            if baseline.get("status") == "ready":
                evaluated.add(baseline_key)
                ...
                if current is not None and median is not None and median >= self.policy.production_min_expected_w:
                    trigger = min(
                    ...
                    if current < trigger:
                        ...
                        deviation = ((current / median) - 1.0) * 100.0 if median else 0.0
                        findings.append(
                            Finding(
                                detector="solar_underproduction",
                                evaluation_key=baseline_key,
                                fingerprint=baseline_key,
                                category="production",
                                severity=severity,
                                confidence=confidence,
                                ...
                                evidence=(
                                    ...
                                    Evidence(
                                        "historical_median",
                                        "Median from comparable local time windows on prior days.",
                                        median,
                                        "W",
                                        "system-history",
                                    ),
                                    Evidence(
                                        "comparable_days",
                                        "Number of prior days contributing to the baseline.",
                                        comparable,
    ...
    async def _record_transitions(self, transitions: list[dict[str, object]]) -> None:
        for incident in transitions:
            ...
            event_type = f"INCIDENT_{transition.upper()}"
            ...
            await self.systems.events_store.record(
                event_type,
                controller_uid=(
                    str(incident["controller_uid"]) if incident.get("controller_uid") else None
                ),
    ...
    async def _evaluate_controller(
        ...
        offline_key = f"controller_offline|{uid}"
        evaluated.add(offline_key)
        status = str(controller.get("status") or "unknown").lower()
        if status != "online":
            findings.append(
                Finding(
                    detector="controller_offline",
                    evaluation_key=offline_key,
                    fingerprint=offline_key,
        ...
        sense = _register_number(sample, "battery_sense_voltage", "battery_remote_sense_voltage")
        terminal = _register_number(sample, "battery_terminal_voltage", "battery_voltage")
        sense_key = f"battery_sense_divergence|{uid}"
        if sense is not None and terminal is not None:
            evaluated.add(sense_key)
            delta = abs(terminal - sense)
            if delta >= self.policy.sense_warning_v:
                findings.append(
                    Finding(
                        detector="battery_sense_divergence",
                        evaluation_key=sense_key,
                        fingerprint=sense_key,
                        category="battery",
                        severity=("critical" if delta >= self.policy.sense_critical_v else "warning"),
                        confidence="high",
                        ...
                        unit="V",
                        evidence=(
                            Evidence("terminal_voltage", "Controller terminal voltage.", terminal, "V"),
                            Evidence("sense_voltage", "Remote battery sense voltage.", sense, "V"),
                        ),
                    )
                )

        input_power = _register_number(
            sample, "input_power", "array_power", "pv_power", "input_power_reported"
        )
        output_power = _register_number(sample, "output_power", "charge_power", "battery_charge_power")
        efficiency_key = f"controller_efficiency|{uid}"
        if (
            input_power is not None
            and output_power is not None
            and input_power >= self.policy.minimum_efficiency_input_w
        ):
            evaluated.add(efficiency_key)
            ...
            if efficiency < self.policy.efficiency_warning:
                findings.append(
                    Finding(
                        ...
                        evaluation_key=efficiency_key,
                        fingerprint=efficiency_key,
                        category="production",
                        severity=(
                            "critical" if efficiency < self.policy.efficiency_critical else "warning"
                        ),
                        confidence="medium",
                        ...
                        unit="%",
                        evidence=(
                            Evidence("input_power", "Controller input power.", input_power, "W"),
                            Evidence("output_power", "Controller charging output power.", output_power, "W"),
                            Evidence(
        ...
        for detector, names, category in (
            ("controller_fault", ("faults", "fault_state"), "communications"),
            ("controller_alarm", ("alarms", "alarm_state"), "communications"),
        ):
            item = _register(sample, *names)
            key = f"{detector}|{uid}"
            if item is None:
            ...
            if not _state_clear(value):
                findings.append(
                    Finding(
                        ...
                        fingerprint=key,
                        category=category,
                        severity="critical" if detector == "controller_fault" else "warning",
                        confidence="high",
                        title=("Controller fault is active" if detector == "controller_fault" else "Controller alarm is active"),
                        summary="A source-backed controller status register reports a non-clear state.",
                        controller_uid=uid,
                        evidence=(
                            Evidence(
        ...
        try:
            polling = await self.systems.controllers_data.polling_summary(uid, window=300, mode="watch")
        except Exception:
            polling = {}
        polling_key = f"polling_degradation|{uid}"
        samples = int(polling.get("samples") or 0)
        if samples >= self.policy.polling_min_samples:
            evaluated.add(polling_key)
            success = float(polling.get("success_rate") or 0.0)
            deadline = float(polling.get("deadline_miss_rate") or 0.0)
            if (
                success < self.policy.polling_warning_success_rate
                or deadline > self.policy.polling_warning_deadline_miss_rate
            ):
                severity = "critical" if success < self.policy.polling_critical_success_rate else "warning"
                findings.append(
                    Finding(
                        detector="polling_degradation",
                        evaluation_key=polling_key,
                        fingerprint=polling_key,
                        category="communications",
                        severity=severity,
                        confidence="high",
                        ...
                        unit="%",
                        evidence=(
                            Evidence("poll_samples", "Recent persisted polling samples.", samples),
                            Evidence("success_rate", "Successful logical polls.", success * 100.0, "%"),
                            Evidence("deadline_miss_rate", "Poll deadline misses.", deadline * 100.0, "%"),
                        ),
                    )
                )

        cycle = await self.charge_cycle_summary(uid, now=now)
        cycle_key = f"charge_stage_cycling|{uid}"
            ...
            if entries >= self.policy.charge_cycle_absorption_entries_warning:
                findings.append(
                    Finding(
                        ...
                        expected_low=0.0,
                        expected_high=float(self.policy.charge_cycle_absorption_entries_warning - 1),
                        unit="entries/24h",
                        evidence=(
                            Evidence(
                                "stage_sequence",
        ...
        coverage_key = f"history_coverage|{uid}"
        start_day = (now.date() - timedelta(days=self.policy.coverage_days)).isoformat()
        end_day = (now.date() + timedelta(days=1)).isoformat()
        try:
            coverage = await self.history.coverage(uid, start=start_day, end=end_day)
        except Exception:
            coverage = {}
        evidence_coverage = coverage.get("daily_evidence")
        if isinstance(evidence_coverage, dict):
            percent = _numeric(evidence_coverage.get("coverage_percent"))
            day_count = int((coverage.get("period") or {}).get("day_count") or 0)
            if percent is not None and day_count >= 3:
                evaluated.add(coverage_key)
                if percent < self.policy.coverage_warning_percent:
                    findings.append(
                        Finding(
                            detector="history_coverage",
                            evaluation_key=coverage_key,
                            fingerprint=coverage_key,
                            ...
                            evidence=(
                                Evidence(
                                    "covered_days",
                                    "Days with local samples or complete retained controller evidence.",
                                    evidence_coverage.get("covered_days"),
                                    source="history-reconciliation",
                                ),
                                Evidence(
                                    "recovered_days",
                                    "Days recovered from complete controller-retained history.",
                                    evidence_coverage.get("recovered_days"),
                                    source="history-reconciliation",
                                ),
                                Evidence(
                                    "missing_days",
                                    "Days with no defensible evidence.",
                                    evidence_coverage.get("missing_days"),
                                    source="history-reconciliation",
                                ),
                            ),
                        )
    ...
    async def solar_baseline(
        ...
        *,
        ...
        metric = latest.get("metrics", {}).get("solar_input_power_w", {})
        if not isinstance(metric, dict):
            metric = {}
        current = _numeric(metric.get("value"))
        start = current_time - timedelta(days=self.policy.production_history_days)
        ...
        for point in history.get("points", []):
            ...
            if timestamp is None or value is None or timestamp.date() == current_time.date():
                continue
            point_minutes = timestamp.hour * 60 + timestamp.minute
            if abs(point_minutes - target_minutes) > self.policy.production_baseline_window_minutes:
                continue
            by_day[timestamp.date().isoformat()].append(value)
        daily_values = [sum(values) / len(values) for values in by_day.values() if values]
        ...
        return {
            ...
            "confidence": (
                "high" if len(daily_values) >= 5 else "medium" if ready else "low"
            ),
    ...
    async def baselines(self, system_uid: str) -> dict[str, object]:
        ...
        cycles = await asyncio.gather(
            *(self.charge_cycle_summary(str(item["controller_uid"])) for item in controllers)
        )
        return {
            "system_uid": system_uid,
    ...
    async def health_score(
        ...
        *,
        ...
        return {
            ...
            "penalties": penalties,
            "semantics": "transparent evidence-backed score; penalties link to active incidents",
        }
